...
export type CatThunk = ThunkAction<
  void,
  AppRootState,
  ThunkExtraArgument,
  Action<string>
...
export function fetchCat(): CatThunk {
  return async function (dispatch, getState, extraArgument) {
    try {
      ...
      const catImage = `${extraArgument.imageRoot}${Math.round(
        Math.random() * 5000
      )}?set=set4&size=100x100`;
      const response = await fetch(
        `${extraArgument.nameRoot}?id=${Math.ceil(Math.random() * 10)}`
      );
